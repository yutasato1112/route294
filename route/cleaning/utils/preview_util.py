...
def language(str_id, lang_id, text):
    ...
    language_dict = {
        'ja': {
            ...
            'meating':'ミーティング',
            'assigned_linen_closet':'担当リネン庫',
            'stock_check':'(在庫確認)',
            'linen_closet_lock':'リネン庫',
            'linen_closet_lock_2':'施錠',
            'key_instructions':'A鍵→EVホール掃除機掛け・飾り棚拭上げ・喫煙所清掃, B鍵→内階段掃除機掛け(連絡統一事項はバインダー確認)',
            'author':'作成者',
            'early_shift':'早番',
        ...
        'en': {
            ...
            'meating':'Meeting',
            'assigned_linen_closet':'Linen Closet',
            'stock_check':'(Stock Check)',
            'linen_closet_lock':'Linen Closet',
            'linen_closet_lock_2':'Lock',
            'key_instructions':'Key A→EV hall vacuuming, shelf wiping, smoking area cleaning, Key B→Inner staircase vacuuming (Check binder for unified notices)',
            'author':'Author',
            ...
            'hotel_name':'RouteInn Shimotsuma',
