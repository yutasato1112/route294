...
class homeView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # ルーミングリスト経由の遷移判定
        if request.POST.get('from_rooming_list'):
            #GETを偽装
            method = 'GET'
            multiple_room_list = request.POST.get('multiple_room_list', '')
            unuse_room_list = request.POST.get('unuse_room_list', '')
            ...
            for floor in room_num_table:
                floor_data = []
                for room in floor:
                    status = '0' if room in unuse_rooms else ''
                    floor_data.append({'room': room, 'status': status})
                combined_rooms.append(floor_data)
        ...
        try:
            ...
            return JsonResponse({'error': 'json file not provided'}, status=400)
